...
class BaseFacebookModel(models.Model):
    ...
    def experiment(self):
        experiment = FacebookUserExperiment.objects.filter(user_id=self.get_user_id())
...
class FacebookUserMovies(models.Model):
    '''
    Model for storing all of a users fb likes
    '''
    # in order to be able to easily move these to an another db,
    ...
    category = models.TextField(blank=True, null=True)
    created_time = models.DateTimeField(blank=True, null=True)
    movie_source = models.TextField(blank=True, null=True)
    movie_about = models.TextField(blank=True, null=True)

    class Meta:
        unique_together = ['user_id', 'facebook_id']


class FacebookUserExperiment(models.Model):
    '''
    Model for storing all of a users fb likes
    '''
    # in order to be able to easily move these to an another db,
    # use a user_id and no foreign key
    user_id = models.IntegerField()
    facebook_id_first = models.BigIntegerField()
    movie_name_first = models.TextField(blank=True, null=True)
    movie_source_first = models.TextField(blank=True, null=True)
    movie_about_first = models.TextField(blank=True, null=True)
    facebook_id_second = models.BigIntegerField()
    movie_name_second = models.TextField(blank=True, null=True)
    movie_source_second = models.TextField(blank=True, null=True)
    movie_about_second = models.TextField(blank=True, null=True)
    user_clicked = models.BigIntegerField(null=True)
    why_movie_like = models.TextField(null=True, blank=True)
    source = models.CharField(max_length=50, null=True,blank=True)
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        unique_together = ['user_id', 'facebook_id_first', 'facebook_id_second']
