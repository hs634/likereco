...
class FacebookUserConverter(object):
    ...
    def get_watched_videos(self, user, limit=5000):
        '''
        Parses the facebook response and returns the likes
        '''
        movie_response = self.open_facebook.get('me/movies', limit=limit)
        movies = movie_response and movie_response.get('data')
        self._store_movies(user, movies)
        self.generate_and_store_random_pairs(user, movies)
        # logger.info('found %s likes', len(watched_videos))
        # print "GETTING WATCHED VIDEOS ..................................."
        # print watched_videos_response.get('data')[0].get('id')
        # movie_id = watched_videos_response.get('data')[3].get('id')
        # movie_details = self.open_facebook.get(movie_id, limit=limit)
        # print "GETTING MOVIE DETAIL..................................."
        # print movie_details.get('source')
    ...
    def get_image_from_google_api(self, movie_name):
        import urllib2
        import simplejson
        url = ('https://ajax.googleapis.com/ajax/services/search/images?v=1.0&q="' + urllib2.quote(movie_name.encode('ascii', 'ignore')) + '"&rsz=5')
